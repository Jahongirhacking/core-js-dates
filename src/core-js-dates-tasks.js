...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (
    Math.floor((new Date(dateEnd) - new Date(dateStart)) / 3600 / 24000) + 1
  );
}
...
function getWeekNumberByDate(date) {
  ...
  const differenceInDays = Math.floor(
    (date - copiedDate) / (1000 * 60 * 60 * 24)
  );
  const weekNumber = Math.floor((differenceInDays + dayOfWeek) / 7) + 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  for (
    ...
    if (isWorkDay) {
      workSchedule.push(formatDate(currentDate)); // Add the current date to the work schedule
      daysCounter += 1;
